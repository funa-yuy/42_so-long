...
/*   Updated: 2025/01/15 03:22:18 by miyuu            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool check(t_data *data, char **maze, int x_i, int y_j) {
	...
	if (x_i < 0 || x_i >= data->player.x || y_j < 0 || y_j >= data->player.y) {
		/* (i,y_j) は迷路外なので通過不可 */
		// ret = false;
		return (false);
	}
...
bool search(t_data *data, char **maze, int s_i, int s_j) {
...
void	bfs_search(t_data *data, char **bfs, int y, int	x)
{
	for (int j=0; j < y; j++){
	printf("%d行目 = {", j);
	for (int i=0; i < x; i++){
	...
	/* スタート位置を(1,3)として開始 */
	if (search(bfs, data->player.y, data->player.x)) {
		printf("ゴールが見つかりました！\n");
	} else {
		printf("ゴールが見つかりません！\n");
	}
...
void	read_map(t_data *data, char *filename)
{
	...
	bfs_search(data, bfs, j, data->x_row);
